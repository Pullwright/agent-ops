...
stage this page holds no backstop for (a stage name `stageBackstopMin` does
not recognise, e.g. `refiner` or `project-reviewer`) reads "no cap on record
for this stage" rather than guessing a direction.
