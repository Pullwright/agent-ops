...
- **Do not close the loop on the originating item.** Unlike `abandoned-drafts`,
  ...
  **not** add a `Fixes #…`/`td-record` block, a `Closes #…`, or a
  changelog entry here; those already happened (or will happen) on the PR's
...
- **Do not close the loop on the originating item.** Unlike `abandoned-drafts`,
  ...
  `Closes #…`, or write a changelog entry here; those already happened
...
- **The originating claim is already made.** The issue — tech-debt or
  ...
  changelog entry in whichever form the repo's `AGENTS.md` directs, exactly
  as for a normal item.
...
- The changelog entry goes where the repo's own `AGENTS.md` documentation
  principles say it goes, and that file — not this summary — decides which
  of two regimes the repo is under. **Under D27** (its `AGENTS.md` says the
  entry lives in the pull-request description; requirement 25c): a
  `## Changelog` heading in the description, then one or more of
  ...
  it, and a malformed section fails whatever the title. Never edit
  `CHANGELOG.md` there: the release pull request assembles it from merged
  descriptions. **Under the file rule** (its `AGENTS.md` still says to add
  an `[Unreleased]` entry to `CHANGELOG.md`): do exactly that, and write no
  `## Changelog` section, since nothing there assembles one. A repo moves
  from the second regime to the first when its adoption lands and its
  `AGENTS.md` changes; follow that file as it reads in your clone.
...
5. **Close the loop on the originating record:**
   ...
   - Human visibility: there is no ledger to flip and no issue to comment
     ...
     further to close.
   - Record the changelog entry as the repo's `AGENTS.md` directs (the
     changelog bullet above): under D27, the description's `## Changelog`
     section — `### Security` for a security fix, which is usually notable
     — or `None.` if the change is not notable; under the file rule, an
